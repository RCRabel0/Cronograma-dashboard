...
def _cor_status_geral_pdf(cor: str) -> tuple[str, str]:
    """Retorna (cor de fundo, cor do texto) em hex para o card de Status Geral."""
    mapa = {
        "verde": ("#D4EFDF", "#145A32"),
        "amarelo": ("#FCF3CF", "#7D6608"),
        "vermelho": ("#F8D7DA", "#78281F"),
    }
    return mapa.get(cor, ("#EAECEE", "#000000"))
...
def gerar_pdf_report_semanal(
    ...
    riscos: list[str],
    marcos_pendentes: list[Tarefa],
    decisoes_pendentes: list[dict],
    observacoes: dict,
) -> bytes:
    """Relatório de uma única página no estilo de um 'report semanal de obra' (status
    geral, KPIs principais, mini Curva S, top avanços/riscos, próximos marcos, decisões
    pendentes e observações) — pensado para impressão ou compartilhamento por e-mail."""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(
        buffer, pagesize=A4,
        topMargin=1 * cm, bottomMargin=1 * cm, leftMargin=1.3 * cm, rightMargin=1.3 * cm,
    )
    estilos = getSampleStyleSheet()
    largura_util = A4[0] - 2.6 * cm
    elementos = []

    elementos.append(Paragraph(f"Report Semanal de Obra — {projeto.nome}", estilos["Title"]))
    elementos.append(
        Paragraph(
            f"Semana {semana_numero} · {periodo_inicio:%d/%m/%Y} a {periodo_fim:%d/%m/%Y} · "
            f"Gestor da obra: {gestor_obra or 'N/D'}",
            estilos["Normal"],
        )
    )
    elementos.append(Spacer(1, 0.4 * cm))
    ...
    estilo_kpi_centro = ParagraphStyle(
        "kpi_centro", parent=estilos["Normal"], alignment=1, leading=12,
    )

    cor_fundo_status, cor_texto_status = _cor_status_geral_pdf(status_cor)
    bloco_status = Table(
        [[Paragraph(
            f"<font size=18 color='{cor_texto_status}'><b>{status_cor.upper()}</b></font>"
            f"<br/><font size=7>{status_descricao}</font>",
            estilo_kpi_centro,
        )]],
        colWidths=[largura_util * 0.27],
    )
    bloco_status.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor(cor_fundo_status)),
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
    ...
    orcamento_total = curva_s["Linha de Base"].max() if len(curva_s) else 0.0
    pct_planejado = 0.0
    if orcamento_total > 0 and len(curva_s):
        data_status_clip = pd.Timestamp(min(max(periodo_fim, curva_s.index[0].date()), curva_s.index[-1].date()))
        pct_planejado = curva_s.loc[data_status_clip, "Linha de Base"] / orcamento_total * 100
    desvio_avanco = indicadores.percentual_concluido - pct_planejado
    pct_desembolso = (indicadores.ac_total / orcamento_total * 100) if orcamento_total > 0 else None

    valores_kpi = [
        f"{indicadores.percentual_concluido:.0f}%",
        f"{indicadores.spi:.2f}" if indicadores.spi is not None else "N/D",
        f"{termino_projetado:%d/%m/%Y}" if termino_projetado else "N/D",
        formatar_valor(indicadores.ac_total, indicadores.unidade),
    ]
    rotulos_kpi = ["Avanço Físico", "SPI (prazo)", "Forecast Término", "Desembolso"]
    sublinhas_kpi = [
        f"Planejado: {pct_planejado:.0f}% · Desvio: {desvio_avanco:+.1f} p.p.",
        ">1,00 = Adiantado · <1,00 = Atrasado",
        f"Baseline: {termino_baseline:%d/%m/%Y} · Atraso: {indicadores.atraso_dias} dia(s)" if termino_baseline else "N/D",
        f"Orçado: {formatar_valor(orcamento_total, indicadores.unidade)}"
        + (f" · Realizado: {pct_desembolso:.0f}%" if pct_desembolso is not None else ""),
    ]
    bons_kpi = [
        desvio_avanco >= 0,
        (indicadores.spi >= 0.95) if indicadores.spi is not None else None,
    ...
    celulas_kpi = [
        Paragraph(
            f"<font size=15><b>{valor}</b></font><br/><font size=6.5>{rotulo}</font>"
            f"<br/><font size=6>{sub}</font>",
            estilo_kpi_centro,
        )
        for valor, rotulo, sub in zip(valores_kpi, rotulos_kpi, sublinhas_kpi)
    ]
    tabela_kpis = Table([celulas_kpi], colWidths=[largura_util * 0.73 / 4] * 4)
    estilo_kpis = [
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
    ...
    for col, bom in enumerate(bons_kpi):
        estilo_kpis.append(("BACKGROUND", (col, 0), (col, 0), _cor_cartao(bom)))
    tabela_kpis.setStyle(TableStyle(estilo_kpis))

    linha_topo = Table([[bloco_status, tabela_kpis]], colWidths=[largura_util * 0.27, largura_util * 0.73])
    ...
    elementos.append(Paragraph("Curva S — Planejado x Realizado", estilos["Heading3"]))
    png_curva = _grafico_curva_s_png(curva_s, indicadores.unidade)
    elementos.append(_imagem_proporcional(png_curva, largura_util / cm, 7))
    elementos.append(Spacer(1, 0.4 * cm))

    estilo_lista = ParagraphStyle("lista_semanal", parent=estilos["Normal"], fontSize=8.5, leading=11)

    bloco_avancos = [Paragraph("Principais avanços", estilos["Heading4"])]
    if avancos:
        for texto in avancos:
            bloco_avancos.append(Paragraph(f"• {texto}", estilo_lista))
    else:
        bloco_avancos.append(Paragraph("Nenhum avanço registrado para a semana.", estilo_lista))

    bloco_riscos = [Paragraph("Principais riscos", estilos["Heading4"])]
    if riscos:
        for texto in riscos:
            bloco_riscos.append(Paragraph(f"• {texto}", estilo_lista))
    else:
        bloco_riscos.append(Paragraph("Nenhum risco identificado no momento.", estilo_lista))

    tabela_avancos_riscos = Table(
        [[bloco_avancos, bloco_riscos]], colWidths=[largura_util * 0.5, largura_util * 0.5]
    )
    tabela_avancos_riscos.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP")]))
    elementos.append(tabela_avancos_riscos)
    elementos.append(Spacer(1, 0.4 * cm))

    bloco_marcos = [Paragraph("Próximos marcos", estilos["Heading4"])]
    if marcos_pendentes:
        dados_marcos = [["Marco", "Data", "Status"]]
        for marco in marcos_pendentes:
            data_marco = f"{marco.termino:%d/%m/%Y}" if marco.termino else "N/D"
            status_marco = "Em andamento" if marco.percentual_concluido > 0 else "Não iniciado"
            dados_marcos.append([marco.nome, data_marco, status_marco])
        tabela_marcos = Table(dados_marcos, colWidths=[largura_util * 0.27, largura_util * 0.1, largura_util * 0.13])
        tabela_marcos.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#2E4053")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
            ("GRID", (0, 0), (-1, -1), 0.4, colors.grey),
        ...
        bloco_marcos.append(tabela_marcos)
    else:
        bloco_marcos.append(Paragraph("Nenhum marco pendente.", estilo_lista))

    bloco_decisoes = [Paragraph("Decisões pendentes", estilos["Heading4"])]
    if decisoes_pendentes:
        dados_decisoes = [["Descrição", "Responsável", "Prazo"]]
        for item in decisoes_pendentes:
            dados_decisoes.append([
                str(item.get("descricao") or ""),
                str(item.get("responsavel") or ""),
                str(item.get("prazo") or ""),
            ])
        tabela_decisoes = Table(dados_decisoes, colWidths=[largura_util * 0.28, largura_util * 0.13, largura_util * 0.09])
        tabela_decisoes.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#2E4053")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
            ("GRID", (0, 0), (-1, -1), 0.4, colors.grey),
            ("FONTSIZE", (0, 0), (-1, -1), 7.5),
        ...
        bloco_decisoes.append(tabela_decisoes)
    else:
        bloco_decisoes.append(Paragraph("Nenhuma decisão pendente registrada.", estilo_lista))

    tabela_marcos_decisoes = Table(
        [[bloco_marcos, bloco_decisoes]], colWidths=[largura_util * 0.5, largura_util * 0.5]
    )
    tabela_marcos_decisoes.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "TOP")]))
    elementos.append(tabela_marcos_decisoes)
    elementos.append(Spacer(1, 0.4 * cm))

    partes_observacoes = []
    if observacoes.get("mao_de_obra"):
        partes_observacoes.append(f"Mão de obra média da semana: {observacoes['mao_de_obra']}")
    if observacoes.get("horas_trabalhadas"):
        partes_observacoes.append(f"Horas trabalhadas (HH): {observacoes['horas_trabalhadas']}")
    if observacoes.get("acidentes") is not None:
        partes_observacoes.append(f"Acidentes com afastamento: {observacoes['acidentes']}")
    if observacoes.get("taxa_frequencia") is not None:
        partes_observacoes.append(f"Taxa de frequência: {observacoes['taxa_frequencia']}")
    if partes_observacoes:
        elementos.append(Paragraph("Observações", estilos["Heading4"]))
        elementos.append(Paragraph(" · ".join(partes_observacoes), estilo_lista))
